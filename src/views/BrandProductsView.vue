<template>
  ...
  <MpDateSettings ref="MpDateSettings" label="Товары бренда" @date-changed="handleDateChange"></MpDateSettings>
  <MpTable
        ...
        return {
            columnNames: [
                ...
                } },
                {
                    headerName: 'Картинка',
                    field: 'img_folder',
                    cellRendererFramework: ImgCell,
                    cellRendererParams: {img_folder: 'img_folder'},
                    autoHeight: true
                },
    ...
    methods: {
        async handleDateChange(startDate, endDate) {
            ...
            if (this.startDate && this.endDate) {
                let data = await this.$mp.ajax("mp-wb-prod", "get-brand-prods", {
                ...
                if (data.error) {
                    alert('Ошибка');
                } else if (data.resp.brand_product) {
                    this.$refs.mpTable.updateData(data.resp.brand_product);
                    this.brandName = data.resp.brand_product[0]["brand"];
                } else {
        ...
        exportToXlsx(data) {
            ...
            this.saveAsXlsxFile(wbout, 'товары бренда.xlsx');
        },
